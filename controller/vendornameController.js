...
const getAllvendor_nameUpdate = async (req, res) => {
  try {
    // Fetch all vendors from VendorNameUpdate
    const vendors = await VendorNameUpdate.find({}).sort({ _id: -1 });
    console.log("Vendors:", vendors);

    // Create a map from vendor names to product IDs
    const vendorProducts = await VendorProducts.find({}).sort({ _id: -1 });
    const vendorNameProductMap = vendorProducts.reduce((acc, vendor) => {
      acc[vendor.name] = vendor.products.map(p => p.productId);
      return acc;
    }, {});
    console.log("Vendor Name to Product Map:", vendorNameProductMap);

    // Check each product for each vendor in VendorNameUpdate
    for (const vendor of vendors) {
      for (const product of vendor.products) {
        const { productId, vendorName } = product;

        // Check if the productId exists in the vendorNameProductMap for the vendor
        if (vendorNameProductMap[vendorName] && !vendorNameProductMap[vendorName].includes(productId)) {
        

          // Delete the product entry from VendorNameUpdate
          await VendorNameUpdate.updateOne(
            { _id: vendor._id },
            { $pull: { products: { productId } } }
          );

          // Optional: If you want to delete the entire vendor if no products are left, you can do that here
          const updatedVendor = await VendorNameUpdate.findById(vendor._id);
          if (updatedVendor && updatedVendor.products.length === 0) {
           
            await VendorNameUpdate.deleteOne({ _id: vendor._id });
          }
        }
      }
    }

    // Send response with the updated vendors data
    const updatedVendors = await VendorNameUpdate.find({}).sort({ _id: -1 });
